...
import { NuxtImg, UCard, UInput, UButton, UModal } from '#components';
import { useAsyncData, useRoute, useI18n, useLazyFetch, useRequestHeaders, definePageMeta, navigateTo, useLocalePath } from '#imports';
...
const route = useRoute("pots-id-contribute___fr");
...
function goBackToPot() {
  navigateTo(localePath({ name: 'pots-id', params: { id: data.value?.id } }));
}
</script>

<template>
  <div v-if="data">
    <UModal :open="true" title="Modal with title" :close="{
      onClick: goBackToPot
    }">
      <template #body>
